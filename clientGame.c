...
void sendUnsignedToServer(int socketServer, unsigned int number){
	unsigned int bytesSend;

	bytesSend = send(socketServer, &number, sizeof(number), 0);

	if (bytesSend < sizeof(number)){
		showError("ERROR mientras se enviaba un unsigned al servidor");
	}
}
...
void sendMessageToServer(int socketServer, char* message){
	...
	// mandamos la longitud del mensaje
	sendUnsignedToServer(socketServer, lenght);

	// mandamos el mensaje
	bytesSend = send(socketServer, message, strlen(message), 0);

	if(bytesSend < strlen(message)){
		showError("ERROR mientras se enviaba un mensaje al servidor");
	}

}

void sendDeck(int socketServer, unsigned int cards [numCards], unsigned int numCards){
...
int main(int argc, char *argv[]){
	...
	tString playerName, rivalName;		/** Name of the player */
	unsigned int code;					/** Code */
		...
		// Init
		// Mandamos el nombre del cliente al servidor
		sendMessageToServer(socketfd, playerName);
		//-------------------------------------------------------------------------------
		// Recibimos el nombre del rival
		memset(rivalName, 0, STRING_LENGTH);
		reciveMessageFromServer(socketfd, rivalName);

		printf("Esta jugando contra %s\n", rivalName);
		//-------------------------------------------------------------------------------
    ...
    // recibe el codigo para saber el estado del jugador y lo muestra
